...
dung_map_2003 <- ggplot() +
  geom_sf(data = ca_counties_sf, 
          color = "darkgrey", 
          fill = "grey", 
          size = 1) +
  geom_sf(data = dung2003, 
          aes(shape = "square"),
          color = "orange", 
          alpha = 0.7, 
          size = 3) +
...
year_plots <- lapply(2003:2022, function(year) {
  dung_year <- dung %>%
...
chin_year_plots <- lapply(2003:2022, function(year) {
  chin_year <- chin %>%
    filter(year == as.character(year))
  ...
  chin_map_year <- ggplot() +
    ...
    geom_sf(data = chin_year, 
            ...
            size = 3) +
    scale_shape_manual(values = 15) + 
    labs(title = paste("Chinook Salmon Distribution -", year)) +
    theme_void()
